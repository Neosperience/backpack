...
class KVSSpyGlass(SpyGlass):
    ...
    def _get_pipeline(self, fps, width, height):
        ...
        pipeline = ' ! '.join([
            f'appsrc',
            f'videoconvert',
            f'video/x-raw,format=I420,width={width},'
...
class KVSCredentialsHandler:
    ...
    def plugin_config_mask(self, plugin_config) -> str:
...
class KVSFileCredentialsHandler(KVSCredentialsHandler):
    ...
    def __init__(self, credentials_path: str='/tmp/credentials.txt', *args, **kwargs):
    ...
    def save_credentials(self, credentials, next_update):
        if next_update is not None:
            ...
            credentials_str = 'CREDENTIALS\t%s\t%s\t%s\t%s' % (
    ...
    def plugin_config(self):
        return ' '.join([
            f'credential-path="{self.credentials_path}"',
        ])
